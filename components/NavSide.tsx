...
export default function NavSide({theme, setTheme, width} : NavSideProps) {
    ...
    return (
        <nav className={`${width < 768 ? 'block relative' : 'hidden'}`}>
            ...
            <div className={`flex flex-col items-center pb-5 absolute w-full z-3 transition-all duration-300 dark:border-t-stone-100 dark:border-b-stone-100 z-3 dark:border-t-1 dark:border-b-1 bg-stone-800 ${navBar ? 'block opacity-100 top-38' : 'opacity-0 top-30 invisible'}`}>
                ...
                (
                    ...
                    >
                    <SunIcon className="h-8 w-8 text-white opacity-0 hidden hover:scale-110 transition-transform hover:rotate-12 dark:inline-block dark:opacity-100 mt-5"></SunIcon>
                    </button>
                ) 
                : 
                (
